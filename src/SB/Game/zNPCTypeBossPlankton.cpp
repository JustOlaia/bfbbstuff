...
void zNPCBPlankton::Reset()
{
    ...
    if (crony == NULL)
    {
        // MODE_HARASS: mode=1, active_territory=0, stun_duration = tweak+0x88
        mode = MODE_HARASS;
        active_territory = 0;
    ...
    {
        // MODE_BUDDY: mode=0, stun_duration = tweak+0x68, newsfish=NULL
        mode = MODE_BUDDY;
    ...
    // Set goal: if MODE_BUDDY -> NPC_GOAL_BPLANKTONIDLE, else vanish + NPC_GOAL_BPLANKTONAMBUSH
    if (mode == MODE_BUDDY)
    {
...
S32 zNPCBPlankton::next_goal()
{
    ...
    return NPC_GOAL_BPLANKTONATTACK + sign;
}
