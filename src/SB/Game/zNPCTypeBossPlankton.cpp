...
void zNPCBPlankton::update_aim_gun(F32 dt)
{
    ...
    F32 distXZ = xVec2Length((xVec2*)&toPlayer);
    F32 yaw = xatan2f(boneMat.pos.y - pLoc->y, distXZ); // elevation angle
...
void zNPCBPlankton::stun()
{
    ...
    switch (numVisible)
    {
    case 1:  sayIdx = 6; break;
    case 2:  sayIdx = 5; break;
    case 3:  sayIdx = 4; break;
    default: sayIdx = 3; break;
    }
    say(sayIdx, 0, false);
}
...
S32 zNPCBPlankton::cronies_dead() const
{
    ...
    for (S32 i = 0; i < cronyCount; i++)
    {
        if (cronies[i] != NULL)
        {
            if (cronies[i]->IsAlive())
