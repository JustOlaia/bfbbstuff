...
void zNPCBPlankton::Reset()
{
    ...
    turn.vel = 0.0f;
    xVec3Copy(&move.vel, &xVec3::create(0.0f, 0.0f, 0.0f));
    ...
    // ASM: beq (crony==NULL) -> MODE_BUDDY path; bne (crony!=NULL) -> MODE_HARASS path
    if (crony == NULL)
    {
        // MODE_BUDDY: active_territory=0, stun_duration=mode_buddy.stun_duration, mode=1
        mode = MODE_BUDDY;
        active_territory = 0;
    ...
    {
        // MODE_HARASS: stun_duration=mode_harass.stun_duration, mode=0, newsfish=NULL
        mode = MODE_HARASS;
    ...
    // Set goal: if MODE_BUDDY -> NPC_GOAL_BPLANKTONATTACK, else (MODE_HARASS) -> NPC_GOAL_BPLANKTONAMBUSH
    if (mode == MODE_BUDDY)
    {
...
U32 zNPCBPlankton::AnimPick(S32 rawgoal, en_NPC_GOAL_SPOT gspot, xGoal* goal)
{
    ...
    U32 offset = (U32)rawgoal - (U32)NPC_GOAL_BPLANKTONIDLE;
    ...
    else
    {
        ...
        case 3:  index = 0x43; break; // STUN
        case 4:  index = 0x46; break; // BEAM
        case 5:  index = 0x49; break; // WALL
...
S32 zNPCBPlankton::next_goal()
{
    ...
    U32 cronyAtk = crony_attacking() & 0xff;
    S32 sign = ((S32)((-(S32)cronyAtk | (S32)cronyAtk) >> 31));
    return NPC_GOAL_BPLANKTONHUNT + sign;
}
...
case 0xD:  t->origin = (zMovePoint*)&obj; break;
case 0xE:  t->timer    = (xTimer*)&obj;           break;
case 0x1B: t->fuse = (zEntDestructObj*)&obj; break;
        break;
    default:
...
void zNPCBPlankton::stun()
{
    ...
    // Play hit sound
    play_sound(SOUND_HIT, (xVec3*)&bound.pad[3], 0.5f);
...
xEnt* platform = territory[active_territory].platform;
if (platform != NULL)
{
    zEntEvent(this, (xBase*)platform, eEventStop);
}
